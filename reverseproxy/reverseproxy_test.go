...
func TestReverseProxy(t *testing.T) {
	...
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		n, err := w.Write([]byte(backendResponse))
		if err != nil {
			t.Errorf("backend write error: %s", err)
		}
		if n != len(backendResponse) {
			t.Errorf("backend write short: got %d want %d", n, len(backendResponse))
		}
	}))
	defer backend.Close()
...
func TestXForwardedFor(t *testing.T) {
	...
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		n, err := w.Write([]byte(backendResponse))
		if err != nil {
			t.Errorf("forward write error: %s", err)
		}
		if n != len(backendResponse) {
			t.Errorf("forward write short: got %d want %d", n, len(backendResponse))
		}
	}))
	defer backend.Close()
	backendURL, err := url.Parse(backend.URL)
